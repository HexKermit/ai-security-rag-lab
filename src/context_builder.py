def build_context(query, top_result, related_results):
    ...
    context.append(f"- Name: {top_vuln['name']} (source: internal dataset)")
    ...
    if related_results:
        ...
        for result in related_results:
            vuln = result["vuln"]
            context.append(
                f"- {vuln['name']} | type={vuln.get('type', 'unknown')} | "
                f"score={result['final_score']:.4f} | source=internal dataset"
            )
        context.append("")
